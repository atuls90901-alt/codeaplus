...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#060606] text-white font-outfit">
      {/* Header */}
      <div className="border-b border-gold/10 px-4 md:px-10 py-4 md:py-5 flex justify-between items-center bg-[#0a0a0a]">
        <div className="font-cormorant text-xl md:text-2xl font-light">CodeaPlus<span className="text-gold">.</span> <span className="text-white/40 text-base md:text-lg">Admin</span></div>
        <div className="flex gap-2 md:gap-3">
          <a href="/" target="_blank" className="font-mono text-[9px] tracking-[.2em] uppercase text-white/40 hover:text-gold transition-colors border border-gold/10 px-3 md:px-4 py-2">Site ↗</a>
      ...
      <div className="flex gap-0 px-4 md:px-10 pt-6 md:pt-8 overflow-x-auto scrollbar-hide">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`font-mono text-[9px] md:text-[10px] tracking-[.15em] md:tracking-[.2em] uppercase px-3 md:px-6 py-3 border-b-2 transition-all flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 ${tab === t.key ? 'text-gold border-gold' : 'text-white/40 border-transparent hover:text-white/70'}`}>
            <span className="hidden sm:inline">{t.label}</span>
            <span className="sm:hidden">{t.short}</span>
            {t.badge ? <span className="bg-gold text-[#060606] text-[8px] px-1.5 py-0.5 rounded-full font-bold">{t.badge}</span> : null}
          </button>
        ))}
      </div>
      ...
      <div className="p-4 md:p-10">
          ...
          <div>
            <div className="flex justify-between items-center mb-6 md:mb-7">
              ...
              <button onClick={() => { setEditProject(null); setShowProjectForm(true) }}
                className="font-mono text-[9px] md:text-[10px] tracking-[.15em] uppercase text-[#060606] bg-gold px-4 md:px-5 py-2 md:py-2.5">
                + Add Project
              </button>
            </div>
            {showProjectForm && (
            ...
            <div className="flex flex-col gap-2">
              {projects.map(p => (
                <div key={p.id} className="flex items-center gap-3 md:gap-5 p-4 md:p-5 border border-gold/10 bg-[#0a0a0a] hover:bg-[#111] transition-colors">
                  <div className="w-12 md:w-16 h-8 md:h-10 rounded flex-shrink-0" style={{ background: p.gradient }} />
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-[14px] md:text-[15px] truncate">{p.title}</div>
                    <div className="font-mono text-[9px] tracking-[.1em] uppercase text-gold mt-0.5">{p.category}</div>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => { setEditProject(p); setShowProjectForm(true) }} className="font-mono text-[9px] tracking-[.1em] uppercase text-white/40 hover:text-gold border border-gold/10 px-2 md:px-3 py-1.5 transition-colors">Edit</button>
                    <button onClick={() => deleteProject(p.id)} className="font-mono text-[9px] tracking-[.1em] uppercase text-white/40 hover:text-red-400 border border-gold/10 px-2 md:px-3 py-1.5 transition-colors">Del</button>
                  </div>
                </div>
              ))}
        ...
        {tab === 'testimonials' && (
          <div>
            ...
            <div className="flex flex-col gap-2">
              {testimonials.map(t => (
                <div key={t.id} className="flex items-start gap-3 md:gap-5 p-4 md:p-5 border border-gold/10 bg-[#0a0a0a] hover:bg-[#111] transition-colors">
                  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center flex-shrink-0 font-cormorant text-base md:text-lg"
                    style={{ background: 'linear-gradient(135deg,#7a6028,#C9A84C)', color: '#060606' }}>
                    {t.avatar_letter}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                      <span className="font-medium text-[14px] md:text-[15px]">{t.author_name}</span>
                    ...
                    <div className="text-[12px] md:text-[13px] text-white/50 font-light line-clamp-2">"{t.content}"</div>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
        ...
        {tab === 'pending' && (
          <div>
            ...
            <div className="flex flex-col gap-3">
              {pending.map(t => (
                <div key={t.id} className="p-4 md:p-6 border border-gold/20 bg-gold/3">
                  <div className="flex justify-between items-start mb-4 gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center font-cormorant text-base md:text-lg flex-shrink-0"
                        style={{ background: 'linear-gradient(135deg,#7a6028,#C9A84C)', color: '#060606' }}>
                        {t.avatar_letter}
                      </div>
                      <div>
                  ...
                  <p className="text-[13px] md:text-[14px] text-white/70 font-light leading-[1.7] border-l-2 border-gold/20 pl-4 mb-4 md:mb-5 italic">"{t.content}"</p>
                  <div className="flex gap-2 md:gap-3 flex-wrap">
                    <button onClick={() => approveReview(t.id)}
                      className="font-mono text-[9px] md:text-[10px] tracking-[.15em] uppercase text-[#060606] bg-gold px-4 md:px-6 py-2 md:py-2.5">
                      ✓ Approve
                    </button>
                    <button onClick={() => rejectReview(t.id)}
                      className="font-mono text-[9px] md:text-[10px] tracking-[.15em] uppercase text-white/40 hover:text-red-400 border border-gold/10 px-4 md:px-6 py-2 md:py-2.5 transition-colors">
                      ✕ Reject
                    </button>
                  </div>
                </div>
              ...
              {pending.length === 0 && (
                <div className="text-center py-20 border border-gold/10">
                  ...
                  <div className="text-white/30 font-mono text-[11px] tracking-widest">No pending reviews.</div>
                </div>
              )}
            </div>
          </div>
        ...
        {tab === 'contacts' && (
          <div>
            <h2 className="font-cormorant text-xl md:text-2xl font-light mb-6 md:mb-7">Contact Submissions <span className="text-white/30">({contacts.length})</span></h2>
            <div className="flex flex-col gap-2">
              {contacts.map(c => (
                <div key={c.id} className={`p-4 md:p-6 border transition-colors ${c.is_read ? 'border-gold/10 bg-[#0a0a0a]' : 'border-gold/30 bg-gold/3'}`}>
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-3">
                    <div>
                      <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                        <span className="font-medium text-[14px] md:text-[15px]">{c.name}</span>
...
function ProjectForm({ initial, onSave, onCancel }: { initial: Project | null, onSave: (d: any) => void, onCancel: () => void }) {
  ...
  const iCls = "w-full bg-[#0a0a0a] border border-gold/10 focus:border-gold text-white font-light text-[13px] px-4 py-2.5 outline-none transition-all"
  return (
    <div className="border border-gold/20 bg-[#0a0a0a] p-5 md:p-7 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
      <h3 className="col-span-1 sm:col-span-2 font-cormorant text-xl font-light mb-2">{initial ? 'Edit' : 'Add'} Project</h3>
      ...
      <div className="col-span-1 sm:col-span-2 flex gap-3 mt-2">
        <button onClick={() => onSave({ ...data, tech_stack: data.tech_stack.split(',').map((s: string) => s.trim()).filter(Boolean) })} className="font-mono text-[10px] tracking-[.15em] uppercase text-[#060606] bg-gold px-6 py-2.5">Save</button>
...
function TestimonialForm({ initial, onSave, onCancel }: { initial: Testimonial | null, onSave: (d: any) => void, onCancel: () => void }) {
  ...
  return (
    <div className="border border-gold/20 bg-[#0a0a0a] p-5 md:p-7 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
      <h3 className="col-span-1 sm:col-span-2 font-cormorant text-xl font-light mb-2">{initial ? 'Edit' : 'Add'} Testimonial</h3>
      ...
      <div className="col-span-1 sm:col-span-2 flex gap-3 mt-2">
        <button onClick={() => onSave(data)} className="font-mono text-[10px] tracking-[.15em] uppercase text-[#060606] bg-gold px-6 py-2.5">Save</button>
        <button onClick={onCancel} className="font-mono text-[10px] tracking-[.15em] uppercase text-white/40 hover:text-white border border-gold/10 px-6 py-2.5 transition-colors">Cancel</button>
      </div>
    </div>
  )
