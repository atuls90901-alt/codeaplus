...
export default function Portfolio({ projects }: { projects: Project[] }) {
  ...
  return (
    <section ref={sectionRef} id="work" className="py-20 md:py-32 px-6 md:px-16 bg-[#060606]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-12 md:mb-16">
        ...
        <a href="#contact" className="reveal-right font-mono text-[11px] tracking-[.15em] uppercase text-white/50 hover:text-gold flex items-center gap-3 group transition-colors relative py-2">
          All Projects <span className="group-hover:translate-x-1.5 transition-transform">→</span>
        </a>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-0.5" style={{background:'rgba(201,168,76,0.1)'}}>
        {projects.map((project, i) => (
          <div
            key={project.id}
            className={`reveal delay-${i%2} relative overflow-hidden bg-[#111] group ${i===0 ? 'col-span-1 sm:col-span-2' : ''}`}
            style={{ aspectRatio: i === 0 ? '16/7' : '16/9' }}
          >
            <div className="absolute inset-0 transition-transform duration-700 group-hover:scale-[1.06]" style={{ background: project.gradient }} />
            <div className="absolute inset-[5%] border border-white/7 rounded-md overflow-hidden bg-black/30 group-hover:[transform:perspective(800px)_rotateX(4deg)_rotateY(-2deg)] transition-transform duration-500"
            ...
            {/* Hover info */}
            <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-9 opacity-0 group-hover:opacity-100 transition-opacity duration-400"
              ...
              <div className="font-cormorant text-2xl md:text-3xl font-normal">{project.title}</div>
              ...
              <div className="font-mono text-[10px] tracking-[.2em] uppercase text-white/40 flex items-center gap-2 mt-2 md:mt-3">
        ...
        {projects.length === 0 && (
          <div className="col-span-1 sm:col-span-2 py-20 text-center text-white/30 font-mono text-sm tracking-widest">
