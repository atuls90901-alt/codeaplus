...
const inputCls = [
  ...
  'focus:border-gold/50 text-white text-[15px]',
  'px-4 py-3.5 outline-none transition-all duration-200',
  'rounded placeholder:text-white/25',
].join(' ')
...
export default function LoginModal({ onClose, onSuccess }: LoginModalProps) {
  ...
  /* Auto-close if already logged in */
  useEffect(() => {
  ...
  const switchTab = (t: 'login' | 'signup') => {
    setTab(t); setStatus('idle'); setErrMsg('')
  }

  const handleEmailAuth = async () => {
    if (!email || !password) {
      setStatus('error'); setErrMsg('Please fill all fields.'); return
    }
    setStatus('loading')
    try {
      if (tab === 'login') {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[10001] flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        {/* Modal box —
            modalUp keyframe defined in globals.css:
        ...
        >
          ...
          <button
            ...
            className="absolute top-4 right-4 w-8 h-8
              ...
              text-white/40 hover:text-white text-base
          ...
          <div className="text-center mb-6">
            ...
            <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-white/40 mt-1.5">
          ...
          <div className="flex border border-gold/10 mb-6 overflow-hidden rounded-sm">
            {(['login', 'signup'] as const).map(t => (
              <button
                key={t}
                onClick={() => switchTab(t)}
                className={`flex-1 py-3 font-mono text-[11px] tracking-[0.2em] uppercase
                  transition-all duration-200
                  ${tab === t
          ...
          {status === 'verify' ? (
            ...
            <div className="flex flex-col gap-4">
              ...
              <p className="text-center font-mono text-[10px] text-white/25 tracking-[0.1em]">
              ...
              {status === 'error' && (
                <div className="font-mono text-[11px] text-red-400
                  border border-red-500/20 bg-red-500/5 px-3.5 py-3 rounded-sm">
                  ✕ {errMsg}
                </div>
              )}
              ...
              <button
                ...
                className="relative overflow-hidden group
                  w-full bg-gold text-bg-primary
                  py-3.5 font-mono text-[12px] tracking-[0.2em] uppercase
                  disabled:opacity-60 disabled:cursor-not-allowed
                  rounded hover:-translate-y-0.5 transition-all duration-300"
              >
              ...
              {/* Switch tab hint */}
              <p className="text-center font-mono text-[10px] text-white/25 tracking-[0.05em]">
